...
// Done: Refactor to not mutate state, but collect and dedupe instead -- need a good way of merging arrays of nested objects
// Refactored using spread operator to update the record
const createOrMergeEntries = (previousValue, currentValue) => {
  const key = Object.keys(currentValue)[0];
  const entry = previousValue[key];
  if (entry) {
    let newOrders = [...entry.orders, ...currentValue[key].orders];
    let orderObject = {};
    newOrders.forEach((order) => {
      if (orderObject[order.order_id]) {
        orderObject[order.order_id].line_items = [
          ...(orderObject[order.order_id].line_items || []),
          ...(order.line_items || []),
        ];
      } else {
        orderObject[order.order_id] = order;
      }
    });
    previousValue[key].orders = Object.values(orderObject);
  } else previousValue[key] = currentValue[key];
